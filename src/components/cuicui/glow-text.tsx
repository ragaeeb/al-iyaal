...
export const GlowText = ({
    ...
    style,
    ...props
}: { children: ReactNode; className?: string; style?: CSSProperties } & ComponentProps<'span'>) => {
    const dropShadows = [
        { blurRadius: 'calc((var(--active, 100) / 100) * 5px)', color: 'hsl(0 0% 100% / 0.6)', offsetX: 0, offsetY: 0 },
        {
    ...
    return (
        <span className={cn('select-none font-medium', className)} style={{ ...style, filter: filterValue }} {...props}>
            {children}
        </span>
    );
};
